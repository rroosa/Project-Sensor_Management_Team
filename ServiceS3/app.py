...
from S3_botoObj import S3BotoObj
...
app.config.from_prefixed_env() # serve per poter exportare da shell un variabile con prefisso FLASK_
...
if app.config["ENV"] == "production":
	...
	try:
		app.config.from_pyfile('config_serviceS3.cfg', silent=False)
...
print("Port App ServiceS3:",p_str)

@app.route('/', methods=['GET'])
def home():
	print("Microservice for managed S3")
	data = jsonify({"Response":f"Microservice ServiceS3 for managing documents in S3 AWS"})
...
def get_Object():
	...
	if response_bool is True:
		...
		#workingdir = os.path.abspath(os.getcwd())
		#app.root_path
		filepath = os.path.join(app.root_path,app.config['DOWNLOAD_FOLDER'],object_name )
		#filepath = workingdir + '/static/download/'
		...
		# AGGIUNGERE as_attachment=True se si vuole SCARICARE
		response = send_file(filepath, mimetype=mimetype, download_name=object_name)
		response.headers["x-filename"] = object_name
		response.headers["Access-Control-Expose-Headers"] = 'x-filename'
		response.headers["Content-Type"] = 'mimetype'
		return response
...
if __name__ == '__main__':
	...
	user_botoObj = S3BotoObj.getInstance()
	...
	id_account_aws = app.config['ID_ACCOUNT_AWS']
	...
	user_botoObj.set_idAccount(id_account_aws)
