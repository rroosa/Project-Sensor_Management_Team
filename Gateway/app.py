...
app.config.from_prefixed_env() # serve per poter exportare da shell un variabile con prefisso FLASK_ (es. FLASK_ENV)
...
if app.config["ENV"] == "production":
	...
	try:
		app.config.from_pyfile('config_gateway.cfg', silent=False)
	except FileNotFoundError as error:
		print(Fore.RED+f"{error}"+Fore.RESET, file=sys.stderr)
		sys.exit()
...
host_s3 = app.config['HOST_S3']
port_s3 = app.config['PORT_S3']
host_DB = app.config['HOST_DB']
port_DB = app.config['PORT_DB']
port_app_gateway = app.config['PORT'] # porta in cui si mette in ascolto l'applicazione flask gateway
port_str = str(port_app_gateway)
print(host_s3, port_s3, host_DB, port_DB, port_str)




# Questo carica prima la configurazione dal modulo config.default_settings
# e poi sovrascrive i valori con il contenuto del file puntata dalla variabile d'ambiente YOURAPPLICATION_SETTINGS. 
# Questa variabile d'ambiente può essere impostata nella shell prima di avviare il server.
...
connect_s3 = f"http://{host_s3}:{port_s3}"
connect_DB = f"http://{host_DB}:{port_DB}"

print(connect_s3, connect_DB)


@app.route('/', methods=['GET'])
def home():
	print("Microservice Gateway")
	return jsonify({"home":f"Microservice Gateway API"})
...
@app.route('/hello/db', methods=['GET'])
def hello_db():
	header = {'ACCEPT': 'application/json'}
	resp = requests.get(connect_DB, headers = header)
	resp_dict = resp.json()
	return jsonify(resp_dict)
...
def show_DataDevice():
	...
	print(format_file)
	if format_file != "csv":
		message_t = "Error in <format_file>, choose (csv)"
		format_file = None
	else:
		message_t = format_file
		#data = jsonify({"Response":message_d})
		#return make_response(data, 707)

	if sensorDevice_model is None or format_file is None:
		data = jsonify({"Response":{"sensorDevice_model":message_d, "format_file":message_t}})
		response = make_response(data, 707)
		response.headers['Content-Type'] = 'application/json'
		return response
	...
	attr_1 = args.get("attr_key_partition")
	op_comparison_1 = args.get("op_comparison_1")
	value_1 = args.get("value_key_partition")

	op_conditional = args.get("op_conditional")

	attr_2 = args.get("attr_key_sort")
	op_comparison_2 = args.get("op_comparison_2")
	value_2 = args.get("value_1_key_sort")
	value_3 = args.get("value_2_key_sort")


	attr_3 = args.get("attr_measure_1")
	op_comparison_3 = args.get("op_comparison_3")
	value_4 = args.get("value_1_measure_1")
	value_4_2 = args.get("value_2_measure_1")

	op_conditional_2_F = args.get("op_conditional_S_F1")

	# clausola 4
	attr_4 = args.get("attr_measure_2")
	op_comparison_4 = args.get("op_comparison_4")
	value_5 = args.get("value_1_measure_2")
	value_5_2 = args.get("value_2_measure_2")

	op_conditional_3_4 = args.get("op_conditional_F1_F2")
	...
	values_C2 = None
	if value_2 is not None and value_3 is not None:
		values_C2 = [value_2, value_3]
	if value_2 is not None and value_3 is None:
		values_C2 = [value_2]
	if value_2 is None and value_3 is not None:
		values_C2 = [value_3]

	if attr_2 is not None and op_comparison_2 is not None and values_C2 is not None:
	...
	values_F1 = None
	if value_4 is not None and value_4_2 is not None:
		values_F1 = [value_4, value_4_2]
	
	if value_4 is not None and value_4_2 is None:
		values_F1 = [value_4]

	if value_4 is None and value_4_2 is not None:
		values_F1 = [value_4_2]


	if attr_3 is not None and op_comparison_3 is not None and values_F1 is not None:
	...
	values_F2 = None
	if value_5 is not None and value_5_2 is not None:
		values_F2 = [value_5, value_5_2 ]
	if value_5 is not None and value_5_2 is None:
		values_F2 = [value_5]
	if value_5 is None and value_5_2 is not None:
		values_F2 = [value_5_2]

	if attr_4 is not None and op_comparison_4 is not None and values_F2 is not None:
		#print(type(value_4)) # string
		clause_4 = {
		"attribute" : attr_4,
...
if __name__ == '__main__':
	...
	print(f"URL S3 is: {connect_s3}")
	print(f"URL DB is: {connect_DB}")
	print(f"BUCKET_TEMPLATE: {app.config['BUCKET_TEMPLATE']}")
	print(f"BUCKET_DATA_SHEET: {app.config['BUCKET_DATA_SHEET']}")
	...
	utility_Obj.set_Url_S3(connect_s3)
	utility_Obj.set_Url_DB(connect_DB)

	
	# port_app_gateway = 5001 di default
	app.run(host='0.0.0.0', port=port_app_gateway, debug=True,use_reloader=False)