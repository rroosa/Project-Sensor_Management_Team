...
class DynamoBotoObj:
	...
	def set_RoleSessionName(self, session_name):
	...
	def parseJsonTemplate( self, file_template, id_device , file_default =None):
		...
		if file_default is not None:
			path_file = file_default
		else:
			path_file = os.path.join(self.root_path,self.folder_template,file_template)
		...
		with open(path_file) as f:
			...
			try:
				...
				print(f"Error convert to dictionary {e}")
				return False , f"Error convert to dictionary for Json Object", 808 # verificato
			else:
		...
		try:
			table = self.DB_resource.create_table(
				...
				ProvisionedThroughput = {
					'ReadCapacityUnits': 5, # lettura 1 unita -> 4KB
					'WriteCapacityUnits': 5 # scrittura 1 unita -> 1KB
				}
			)
	...
	#--------------------------------------------------------------
	def writeData(self, id_device, path_dataset_json, table_default = None):
		...
		try:
			...
			mess = f"File error: {e}"
			return "File NotFound Error",707

		except json.JSONDecodeError as e:
			print(Fore.RED+ f"{e}"+Fore.RESET)
			return  f"Error Decode JSON file", 808 

		except AttributeError as error:
			print(Fore.RED + f"{e}"+Fore.RESET)
			return f"AttributeError in Localfile",808
			...
			try:
				if table_default is not None:
					...
					with table_default.batch_writer() as writer:
				...
				mess = f"Error Param Validation, it is requests the list of elements"
				code = 808
				print(Fore.RED+ f"Error Param Validation"+Fore.RESET)
				return mess, code
	...
	## 			DELETE TABLE
	#-----------------------------------------------------------------
	def deleteTable(self, table_name, table_default=None):
		print(f"Delete table [{table_name}] from DynamoDB")
		try:
			if table_default is None:
				resp = self.DB_client.delete_table(
					TableName = table_name
					)
			else:
				table_default.delete()
	...
	##
	#-----------------------------------------------------------
	def prepare_send_Query(self, id_device, list_clause_where, list_op_conditional, list_clause_filter,list_op_conditional_filter,select, table_default = None):
		#print(list_clause_where)
		...
		try:
			...
			if keyConditionExpression == "":
				return "Error, no correct (op_comparison) for Key Partition", 606
			...
			print(Fore.RED+f"Error formulate Query in clause key Partition"+Fore.RESET)
			return f"Error formulate Query in clause key Partition", 606

		except ClientError as error:
			#print(f"Error delete table [{table_name}], {error.response['Error']['Message']}")
			mess = error.response['Error']['Message'] # Requested resource not found
			code = error.response['Error']['Code']
			print(Fore.RED + f"Mess:{mess}, Code: {code}"+ Fore.RESET)
			...
			print(Fore.RED + f"Error type for attribute: {e}"+Fore.RESET)
			return "TypeError in value of attribute", 606
	...
	def prepare_send_Scan(self, id_device, list_clause_filter, list_op_conditional_filter, select, table_default = None):
		...
		try:
			...
			print(Fore.RED + f"TypeError in value of attribute: {e}"+Fore.RESET)
			return "TypeError in value of attribute", 606
