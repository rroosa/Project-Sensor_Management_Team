...
def prepare_Cond_Att(list_clause_key,list_op_conditional, list_clause_filter, list_op_conditional_filter):
	print("QUERY")
	print(list_clause_key)
	print(list_op_conditional)
	if len(list_clause_key)== 2 and len(list_op_conditional) == 0:
		list_op_conditional.append("and")

	if len(list_clause_filter)==2 and len(list_op_conditional_filter)==0:
		list_op_conditional_filter.append("and")

	if len(list_clause_filter)==3:
		if len(list_op_conditional_filter)==0:
			list_op_conditional_filter.append("and")
			list_op_conditional_filter.append("and")
		elif len(list_op_conditional_filter)==1:
			list_op_conditional_filter.append("and")
	...
	for count, clause in enumerate (list_clause_key, start = 1 ):
		...
		if 'between' == comparison and len(clause_values) == 2: # BETWEEN  :attr_{i} AND :attr_{i+1}
			...
			if is_decimal(clause_values[1]):
				clause_values[1] = Decimal(clause_values[1])
			...
			print(list_value)
			for k, el in enumerate(list_value):
				if is_decimal(el):
					list_value[k] = Decimal(el)
