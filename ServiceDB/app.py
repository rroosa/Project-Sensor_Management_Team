...
if app.config["ENV"] == "production":
	...
	try:
		app.config.from_pyfile('config_servicedb.cfg', silent=False)
...
port_app_DB = app.config['PORT']
p_str = str(port_app_DB)
print("Port App ServiceDB:",p_str)

@app.route('/', methods=['GET'])
def home():
	print("Microservice ServiceDB for data management in DynamoDB AWS")
	data = jsonify({"home":f"Microservice ServiceDB for managing data in DynamoDB AWS"})
...
if __name__ == '__main__':
	...
	db_botoObj.setRootPath(app.root_path)
	...
	app.run(host='0.0.0.0', port=port_app_DB, debug=True, use_reloader=False)
